...
if( ! class_exists( 'CED_SADC_DIR' ) )
{
	class CED_SADC_DIR
	{
		...
		public function __construct()
		{
			...
			add_filter( 'woocommerce_get_item_data', array ($this, 'ced_sadc_add_item_meta' ), 10, 2 ); //it will add field in single product
			add_filter( 'woocommerce_add_cart_item_data', array($this, 'ced_sadc_add_cart_item_data'), 10, 2); //by this product data save on submit single page
			...
			add_action( 'woocommerce_before_add_to_cart_form', array($this, 'ced_sadc_check_product_options'));  // modal data form
    ...
    */
		...
		function ced_sadc_show_hide_js()
		{
			...
			{
				wp_enqueue_script( 'ced_sadc_show_hide_js', CED_SADC_DIR_URL.'/assets/js/ced_sadc_show_hide.js', array('jquery'));
			}
			...
			if(isset($sadc_setting['ced_sadc_enabled']))
			{
				...
				<!-- MOdal code of cart page -->
					...
					<div class="close-btn">
						...
						<input type="hidden" name="ced_sadc_rec_add" id="ced_sadc_rec_add" value=""> <!-- All modal data set in this value by js-->
						</div>
		 ...
		 */
		 ...
		 {
			include_once CED_SADC_DIR.'/admin/settings.php';  //CED_SADC_DIR is define in main file
		 } 
		...
		/**
		 * This function is used add setting field data to cart.And this function work after submit in single page
		...
		function ced_sadc_add_cart_item_data($the_cart_data, $product_id)
		{
					...
					if(isset($address))   //this all html data show in cart page in this html struct
					{
		...
		/**
		 * This function is used to save field with order.it work on click 'place order'
